...
iupdate(struct inode *ip)
{
  ...
  dip->nlink = ip->nlink;
  dip->size = ip->size;
  // copy new P5 data from inode to dinode
#ifdef CS333_P5
...
#ifdef CS333_P5
// looks up path and updates inode to new mode
int
chmodHelper(char* path, int mode)
{
  struct inode* inode;

  begin_op();
  // path lookup failure case
  if ((inode = namei(path)) == 0) {
    end_op();
    return -1;
  }
  ilock(inode);
  inode->mode.asInt = mode;
  iupdate(inode);
  iunlock(inode);
  end_op();
  return 0;
...
// looks up path and updates inode to new uid
int
chownHelper(char* path, int uid)
{
  struct inode* inode;
...
// looks up path and updates inode to new gid
int
chgrpHelper(char* path, int gid)
{
  struct inode* inode;
